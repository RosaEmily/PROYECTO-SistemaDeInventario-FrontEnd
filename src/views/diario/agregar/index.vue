...
export default {
    ...
    data() {
        return {
            ...
            fieldsDetalle: [
                ...
                { key: "det_descripcion", label: "Concepto", sortable: false },
                { key: "det_id_cta", label: "CtaContable", sortable: false },
    ...
    methods: {
        ...
        async getConfiguraciones() {            
            ...
            let request3 = {
                url: "/api/plantillaDiario/" +  localStorage.getItem("idPlantillaDiario"),
                method: "GET",
            };
            var respRoles = await store.dispatch("back/EXECUTE", request3);
            console.log(respRoles)
            this.otro.lib_glosa = respRoles.pl_glosa;
            this.otro.lib_moneda= respRoles.pl_moneda;
            if(this.otro.lib_moneda!="PEN"){
                if (!this.consulta) {
                    axios
            ...
            respRoles.Detalle_Plantilla_Diario.forEach(e => {
                this.otro.detalle_operacion.push({
                    det_nro_cta:e.det_nro_cta,
                    det_banco:e.det_banco,
                    det_nro_doc:"",
                    ...
                    det_nro_cta_banco:e.det_nro_cta_banco,
                    det_debe_haber:e.det_debe_haber,
                    det_pago:e.det_pago,
        ...
        Cancelar() {
            this.$router.push({ name: "diario-lista-index" });
        },
        async saveOtro() {
            let request = {
                url: "/api/diario/",
            ...
            try {
                ...
                if (respRoles.status == 200) {
                    this.sendMessage("Diario registrada satisfactoriamente",'EditIcon',"success");
                    this.$router.push({ name: "diario-lista-index" });
        ...
        hideNroCuenta(){
            if(this.nro_cuenta!=false){
                this.otro.detalle_operacion[this.index].det_nro_cta_banco=this.nro_cuenta;
            }else{
                this.otro.detalle_operacion[this.index].det_nro_cta_banco=null;
            }
                    ...
                    >                        
                        ...
                        >
                            <b-form-select
                                ...
                                @change="item.det_debe=0,item.det_haber=0,sumaTotal()"
                                :options="debehaber"
                            />
                        </template>                 
                        <template
                        ...
                        >
                            <b-input-group>
                                <b-form-input
                                    ...
                                    type="number"
                                    @keypress="onlyNumbers($event)"
                                    :readonly="item.det_debe_haber != 'D'"
                                    @keyup="sumaTotal()"
                                />                                
                            </b-input-group>
                        </template>
                        <template
                            v-slot:cell(det_haber)="{
                                item,
                                field: { key },
                            }"
                        >
                            <b-input-group>
                                <b-form-input
                                    ...
                                    type="number"
                                    @keypress="onlyNumbers($event)"
                                    :readonly="item.det_debe_haber != 'H'"
                                    @keyup="sumaTotal()"
                                />                                
                            </b-input-group>
                        </template>                                                                      
