...
export default {
    ...
    methods: {     
        ...
        async getInfoByID() {
            let request = {
                url: "/api/plantillaCajaBanco/" + this.$route.params.id,
                method: "GET",
            };
            ...
            this.plantilla.pl_filtro = resp.pl_filtro;
            this.plantilla.pl_moneda = resp.pl_moneda;
            this.plantilla.pl_tipo = resp.pl_tipo;
            this.tipo = resp.pl_tipo;
            this.plantilla.emp_id = resp.emp_id;
            this.plantilla.detalle_plantilla = resp.Detalle_Plantilla_Caja_Bancos;          
        },
            ...
            this.$refs.simpleRules.validate().then((success) => {
                if (success) {
                    if (this.plantilla.detalle_plantilla.length > 0){
                        if(this.plantilla.pl_tipo=="1"){
                            for(let i=0; i<this.plantilla.detalle_plantilla.length; i++){
                                if(this.plantilla.detalle_plantilla[i].det_id_cta == null || this.plantilla.detalle_plantilla[i].det_moneda == null
                                || this.plantilla.detalle_plantilla[i].det_nro_cta_banco == null
                                || this.plantilla.detalle_plantilla[i].det_pago == null || this.plantilla.detalle_plantilla[i].det_codigo_efecto == null
                                ){
                            ...
                            for(let i=0; i<this.plantilla.detalle_plantilla.length; i++){
                                ...
                                || this.plantilla.detalle_plantilla[i].det_banco == null ){
                                    this.sendMessage("Complete todos los datos requeridos en la tabla detalle de su plantilla","AlertTriangleIcon","danger");
                                    return false;
                                }
                            }
        ...
        mensajeConfirmacion(){
            ...
            if(this.plantilla.pl_tipo=="1"){
                ...
                for(let i=0; i<this.plantilla.detalle_plantilla.length; i++){
                    if(this.plantilla.detalle_plantilla[i].det_id_cta == null && this.plantilla.detalle_plantilla[i].det_debe_haber == null 
                    && this.plantilla.detalle_plantilla[i].det_nro_cta_banco == null && this.plantilla.detalle_plantilla[i].det_codigo_efecto == null
                    && this.plantilla.detalle_plantilla[i].det_banco == null && this.plantilla.pl_moneda==null){
                        aux=false;
                    }
                }
            }    
            if(aux){
                this.$swal({
                    title: "¿Está seguro que quieres cambiar de tipo de plantilla, por que perderas todo tu progreso?",
                    text: "",
                    icon: "warning",
                    ...
                    if (result.value) {
                        this.plantilla.pl_tipo=this.tipo,
                        this.plantilla.pl_filtro=null,
                        this.plantilla.pl_moneda=null,
                        this.plantilla.detalle_plantilla=
                        [
                            {
                                det_id_cta: null,
                        ...
                        if(this.tipo=="1"){
                            this.tipo="2";
                            this.plantilla.pl_tipo="2";
                        }else{
                            this.tipo="1";
                            this.plantilla.pl_tipo="1";
                        }
                    }
                });
            }else{
        ...
        Guardar() {
            ...
            this.savePlantilla();
            this.$router.push({ name: "caja-plantilla" });
        },
        Cancelar() {
            this.$router.push({ name: "caja-plantilla" });
        },
            ...
            <validation-observer ref="simpleRules">
                <b-form class="ml-1 mr-1 mt-1">                   
                    <b-row class="">
                        ...
                        <b-col sm="6" v-if="plantilla.pl_tipo=='1'">
                            <b-form-group label="Filtro de Documento: ">
                                <validation-provider
                                    #default="{ errors }"
                                    name="Filtro de Documento"
                                    rules="required"
                                >
                                    <b-input-group>
                                        <b-form-select
                                            v-model="plantilla.pl_filtro"
                                            class="form-control"
                                            :options="filtros"                                        
                                        />
                                    </b-input-group>
                        ...
                        <b-col sm="6" v-if="plantilla.pl_tipo=='2'">
                        ...
                        <b-col sm="12" v-if="plantilla.pl_tipo=='1'">
                            <b-table
                                striped
                                responsive
                                select-mode="range"
                        ...
                        <b-col sm="12" v-if="plantilla.pl_tipo=='2'">
                            <b-table
                                striped
                                responsive
                                select-mode="range"
