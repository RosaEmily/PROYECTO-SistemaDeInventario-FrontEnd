...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async getInfoByID() {
                let request = {
                    url: "/api/categoria/" + this.$route.params.id,
                    method: "GET",
                };
                var respRoles = await store.dispatch("back/EXECUTE", request);
                this.categoria= respRoles;
            },
            cancelar() {
            ...
            async saveCategoria() {
                ...
                try {
                    var respRoles = await store.dispatch("back/EXECUTE", request);
                    if (respRoles == 201) {
                        this.sendMessage("Categoria editado satisfactoriamente","CheckSquareIcon","success");
                        this.$router.push({ name: "inventario-categoria-listar-index" });
                    } else if (respRoles == 400) {
                        this.sendMessage("La categoria que intenta editar ya existe","AlertTriangleIcon","danger");
                    } else {
