                    ...
                    <b-row>
                        ...
                        <b-col sm="6">
                            <b-form-group label="Cuenta Contable: ">
                                <b-form-input
                                    :value="
                                        ctaBancoData.cb_nro_cta +
                                        ' - ' +
                                        ctaBancoData.cb_desc_cta
                                    "
                                    :readonly="true"
                                />
                            </b-form-group>
...
export default {
    ...
    methods: {
        async getInfoByID() {
            let request = {
                url: "/api/cuenta_bancaria/" + this.$route.params.id,
                method: "GET",
            };
            var respRoles = await store.dispatch("back/EXECUTE", request);
            console.log(respRoles);
            this.ctaBancoData = respRoles;
        },
        validationForm() {
            ...
            try {
                ...
                if (respRoles.status == 500) {
                    ...
                    if (respRoles.status == 400) {
                        ...
                        this.sendMessage(
                            "Cuenta Bancaria editada satisfactoriamente",
                            "EditIcon",
                            "success"
                        );
                        this.$router.push({
